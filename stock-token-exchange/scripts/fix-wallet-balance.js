...
async function main() {
  try {
    ...
    console.log(`Owner's token balance: ${ethers.utils.formatEther(ownerBalance)} ${symbol}`);
    
    const bobBalance = await stockToken.balanceOf(bob.address);
    console.log(`Bob's token balance: ${ethers.utils.formatEther(bobBalance)} ${symbol}`);
    
    // If Bob has no tokens, mint some
    if (bobBalance.eq(0)) {
      console.log("Bob has no tokens! Attempting to mint 1000 tokens to Bob's address...");
      const tx = await stockToken.connect(owner).mint(bob.address, ethers.utils.parseEther("1000"));
      await tx.wait();
      console.log("Tokens minted! Transaction hash:", tx.hash);
      
      // Verify the new balance
      const newBalance = await stockToken.balanceOf(bob.address);
      console.log(`Bob's new token balance: ${ethers.utils.formatEther(newBalance)} ${symbol}`);
    }
    ...
    console.log(`Bob's allowance for the exchange: ${ethers.utils.formatEther(allowance)} ${symbol}`);
    ...
    if (allowance.lt(ethers.utils.parseEther("1000"))) {
      ...
      console.log(`Bob's new allowance for the exchange: ${ethers.utils.formatEther(newAllowance)} ${symbol}`);
    }
    ...
    try {
      ...
      console.log(`Bob's exchange token balance: ${ethers.utils.formatEther(exchangeTokenBalance)} ${symbol}`);
    } catch (error) {
    ...
    if (exchangeTokenBalance.lt(ethers.utils.parseEther("100"))) {
